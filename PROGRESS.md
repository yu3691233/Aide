...
### 当前版本：0.9.17（Windows 产品化发布候选）

- **版本号**：`version.json` = 1.0.7；Android `versionCode` = 78，`versionName` = "1.0.7"
...
| IDE 激活后输入框聚焦 | `inject_to_ide.py`、输入框校准配置 | ✅ Trae / AGY 优先直接最大化恢复原生焦点；失败时才按已有校准点点击兜底 |
| 管理员 IDE 自动控制 | `windows_privilege.py`、`ide_process_control.py`、`dispatch_utils.py`、IDE 启停路由 | ✅ 检测窗口完整性级别；派发、启动、关闭按需走受限 RunAs 操作，普通权限 IDE 保持原路径 |
...
| MCP 主 IDE 经理模式准备 | `mcp_server.py` | ✅ 生成紧凑任务包与 IDE 候选；优先推荐已打开空闲员工 IDE；保留主 Codex 完成和 Codex 新会话接力；仅在用户明确确认后派发，并阻止代码文件范围冲突 |
| MCP 经理/员工反馈闭环 | `mcp_server.py`、`skills/aidelink-manager-worker/` | ✅ 辅助 IDE 获取限定任务包，成功必须携带 result_ref、失败单独回传；主 IDE 独立验证后才可标记 done；Codex / Trae 安装 MCP 时同步安装协作技能 |
| IDE 可见运行态识别 | `dispatch_utils.py` | ✅ 仅将拥有可见窗口的目标 IDE 可执行文件视为已打开，排除 minimax-mcp 等同名辅助进程 |
| ChatGPT → Codex 紧凑接力探针 | `plugins/aidelink-handoff/`、`.agents/plugins/marketplace.json` | ✅ 八字段接力包、UTF-8 深链、解码完整性、长度矩阵和复制兜底；只预填不自动发送，普通 Chat 插件可见性仍按客户端实测 |
| MCP 委派任务证据死锁恢复 | `task_monitor.py`、`mcp_server.py` | ✅ `result_ref_required` 的只读/空文件范围任务不再被旧提交自动完成；合法代码提交自动携带 `commit:` 证据；历史 `pending_test + result_ref=null` 可安全补报或失败释放，已有证据禁止覆盖 |
| MiniMax Code 定位为 Skill-only 员工 | `routes/ide_routes.py`、`static/js/config.js` | ✅ 移除虚假 MCP 安装支持：`/api/ide/install-mcp` 不再为 `minimax` 写入任何 mcp.json，返回 `success:false` 与明确非永久性提示；IDE 管理卡片对 MiniMax 改为不可点击的 🧩 Skill 模式 标签。MiniMax Code 不参与 AideLink MCP 自动任务闭环，任务和结果由用户人工转发；未来官方开放第三方 MCP 后再重新验证接入 |
...
| 灵感创建兜底 | `ChatViewModel.kt`, `OfflineTaskCache.kt` | ✅ 未成功创建/派发时统一保存为当前项目的本地灵感；选择 IDE 后同步派发，失败则继续保留 |
| 项目级灵感与跨 IDE 任务筛选 | `OfflineTaskCache.kt`、`TaskListPanel.kt`、`ChatViewModel.kt` | ✅ “全部”展示当前项目所有 IDE 任务；本地灵感按项目隔离且不绑定 IDE，派发时再选择目标；卡片标题显示项目名，版本优先显示 AideLink 版本 |
...
| 2026-07-16 | 修复截图监控内存增长：WinRT/GDI/PIL 截图资源在 JPEG 编码后立即释放，截图请求串行化；Android 监控移除未使用的重复全图解码并回收缩放前 Bitmap | Codex |
| 2026-07-16 | 完成 MCP 主 IDE 经理模式最小闭环：紧凑任务包、打开空闲 IDE 推荐、主 Codex / Codex 新会话选项、显式确认派发、文件范围冲突保护和 result_ref 优先回传契约 | Codex |
| 2026-07-16 | 修复 MiniMax MCP 辅助进程误报 IDE 已打开；完善 AideLink 经理/员工技能、Trae Solo CN 安装、证据回传、失败回传及主 IDE 验证完成闭环 | Codex |
| 2026-07-19 | 统一三端任务状态与平台标记，完善紧凑浮窗、FRP 生命周期、待测试反馈及任务编辑流程；版本升级到 1.0.3（versionCode 74） | Codex |
| 2026-07-19 | 任务派发 fallback：未分配任务在所有 IDE 视图显示，浮窗创建任务指派当前选中 IDE；新增 Codex 周配额徽章（5 分钟轮询 + 点击刷新） | Trae |
| 2026-07-19 | 修复 Codex desktop IDE 探测：改用 PowerShell Get-AppxPackage 查询 WindowsApps 路径，无需管理员权限 | Trae |
| 2026-07-19 | 修复 wake_screen 误触 manager 重启：移除鼠标右键 + Enter 组合，仅保留 SetThreadExecutionState + 鼠标相对移动 | Trae |
| 2026-07-19 | 修复关显示器后派发失败：ensure_screen_unlocked 改为无条件调用 wake_screen；新增 App 关闭显示器功能与 /screen/off 端点 | Trae |
| 2026-07-19 | 修复方向锁定被关闭：server 安装后用 monkey 启动 App 会注入系统事件触发旋转，改用 cmd package resolve-activity + am start；版本升级到 1.0.4（versionCode 75） | Trae |
| 2026-07-23 | 项目界面地图接入运行态优先采集：Android 复用 UiAutomator，Windows 使用 UI Automation/原生控件树并回退源码扫描；浮窗组件定位可直接消费运行态边界与标识 | Codex |
| 2026-07-23 | 浮窗创建任务改为项目级“修改目标池”：组件可长期保留、一键插入底部输入框；智能提示词按钮在未选目标时先从地图推荐候选，选定后再按需优化表达 | Codex |
```
