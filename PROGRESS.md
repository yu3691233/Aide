...
### 当前版本：0.9.13（Windows 产品化发布候选）

- **版本号**：`version.json` = 0.9.13；Android `versionCode` = 66，`versionName` = "0.9.13"
...
| 离线任务创建兜底 | `ChatViewModel.kt`, `OfflineTaskCache.kt` | ✅ 健康状态滞后或服务端创建失败时自动保存本地，恢复连接后同步 |
...
| 2026-07-14 | 修复 Android 离线任务完整入口：服务端失败时回退缓存，断网且目标为 Aide 时仍可创建并查看待同步任务 | Codex |
